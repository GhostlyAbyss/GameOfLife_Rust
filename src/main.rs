...
fn main() -> eframe::Result {
    ...
    eframe::run_native("My Game of Life App", options, Box::new(|cc| Ok(Box::new(GameOfLifeApp::new(cc)))))
}
        ...
        egui::TopBottomPanel::top("Controls").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if ui.button("Generate Random").clicked() {
                    self.game = GameOfLife::generate_game(rng().random_range(0..60), rng().random_range(0..60), rng().random());
                }

                if ui.button("Generate Blank").clicked(){
                    self.game = GameOfLife::generate_game(rng().random_range(0..60), rng().random_range(0..60), 0.0);
                }
            });
        });
